    ...
    <div class="row">
      <div class="col-12">
        <div class="card">
          ...
          <!-- /.card-header -->
          <div class="card-body">
            <?php echo form_open_multipart('invoice/purchases/entry/download', [ 'class' => 'form-validate', 'autocomplete' => 'off' ]); ?>
              <div class="row">
                <div class="col-4">
  ...
  $(function() {
    ...
    var table = $("#example2").DataTable({
      ...
      columns: [{
        ...
        {
          data: "user_purchasing_update_by",
          orderable: false,
          visible: false,
          render: function(data, type, row) {
            return `${data?shorttext(data, 12, true):''}`;
            return `<a href="${location.base}users/view/${row['user_id']}">${data}</a>`;
            return `<a href="${location.base}users/view/${row['user_id']}">${shorttext(data, 12, true)}</a>`;
          }
        },
