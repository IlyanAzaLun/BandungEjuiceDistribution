...
class Login extends CI_Controller
{
	...
	{
		$this->load->view('account/reset_login', $this->data, FALSE);
	}

	public function update_reset_login()
	{
		$this->form_validation->set_rules('username', 'Username', 'trim|required|min_length[5]|xss_clean|callback_validate_username');
		if ($this->form_validation->run() != FALSE) {
			$username = post('username');
			...
			$update = ['is_logged' => 0];
			$result = $this->users_model->update($user->id, $update);
			$this->activity_model->add("Update status loggin, #" . $result, (array) $user);
		}
		$this->session->set_flashdata('message', 'Status logg has been Updated, You can login now');
		$this->session->set_flashdata('message_type', 'success');
		redirect('/');
	}
}
